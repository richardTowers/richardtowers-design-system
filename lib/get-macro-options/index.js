...
function getMacroOptionsJson (componentName) {
  try {
    let optionsFilePath = path.join(paths.richardtowersfrontendcomponents, componentName, 'macro-options.json')
    return JSON.parse(fs.readFileSync(optionsFilePath, 'utf8'))
  } catch (err) {
    // console.error(err)
    // TODO: This is broken, because in the master branch of govuk-frontend we're using YAML instead of JSON to describe the options.
    return []
  }
}
